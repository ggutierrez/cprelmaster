...
#+LATEX_HEADER: \newcommand{\oobo}{\ensuremath{\operatorname{oob}}}
#+LATEX_HEADER: \newcommand{\oob}[1]{\ensuremath{\oobo(#1)}}
...
** Verbatim text
   ...
   #+END_EXAMPLE
** Long table
   Use at the beggining of the table:
   ATTR_LaTeX: longtable
** Tables
*** Column formula:
    TBLFM: $8=$5/(1024*1024);%.2f
...
*** Example
    - To represent a relation as a binary decision diagram we have to
      establish some encoding first.
    - The reason for this encoding is that a /Bdd/ is able to
      represent functions of the form
      $F(x_0,x_1,\ldots,x_n)\rightarrow \{0,1\}$, where $x_i \in \{0,1\}$.
    - An straightforward way of encoding a tuple of integers
      $\tuple{k}$ is by translating every element of the tuple into
      its binary representation and by /selecting/ a subset of variables
      in the /Bdd/ to represent it.
    - Then, assumming that we need $R$ bits to represent the maximum
      possible element, we will need $R\times C$ /Bdd/
      variables. Where $C$ is the number of columns that the relation
      has. As an example consider that we want to represent a binary
      relation with the maximum element being 3. The following table
      shows some pairs and their encodings.
      |-------------+-------+-------+-------+-------|
      ...
      | \pair{1}{2} |     1 |     0 |     0 |     1 |
      |-------------+-------+-------+-------+-------|
    - In the encoding above we use 4 /Bdd/ variables, two per each
      element in a pair. This is because the maximum element which is
      3 can be represented with 2 bits.
    - Columns $V_0$ and $V_2$ represent the first element in the pair,
      being $V_0$ the least significant bit. In the same way $V_1$ and
      $V_3$ represent the second element of the pair, being $V1_$ the
      least significant.
    - Other orders in the variable representation are possible. We
      chose this one as is the one offering better compression during
      our experiments.
...
** Constraint model
*** Input
    The input of the problem consists of three /ground relations/ and
    three set of packages:
    - Deps :: : This is the set of all the dependencies as
                      they are read from the /KCUDF/ document that the
                      reducer emits.
    - Confs :: : The relation containing the conflicts.
    - Provs :: : The relaion containing the provides.
    - Packages :: : A set with all the packages in the system.
    - MustI :: : A set of packages that /must/ be
                     installed because of the request.
    - MustU :: : A set of packages that ust be uninstalled.


**** TODO Talk about the fact a concrete package provides itself and the dependency provides

*** Variables and their domains
    |----------+----------------------------------------------+--------------------------------------------------------+--------|
    | Variable | Lower bound                                  | Upper bound                                            | Kind   |
    |----------+----------------------------------------------+--------------------------------------------------------+--------|
    | $DEPS$   | $\operatorname{Deps}\cup\operatorname{Provs}$ | $\universe{2}$                                         | RelVar |
    | $CONFS$  | Confs                                        | $\universe{2}$                                         | RelVar |
    | $PROVS$  | $\emptyrel{2}$                               | Provs                                                  | RelVar |
    | $INST$   | MustI                                        | $\operatorname{Packages}\setminus\operatorname{MustU}$ | RelVar |
    |----------+----------------------------------------------+--------------------------------------------------------+--------|    

** Constraints
*** Ensure dependencies
    The dependency of every installed package are ensured by the
    dependency constraint. This constraint takes as arguments the
    dependency variable and the installation variable. The propagation
    it performs is:
    - The dependencies of all installed packages are also part of the
      installation.
      \begin{align*}
      \operatorname{ToConsider} &= (\glb{\operatorname{INST}}\times \universe{})\cap\glb{\operatorname{DEPS}} \\
      \operatorname{Needed} &= \proj{\operatorname{ToConsider}}{1}\\
      \operatorname{Needed} \subseteq \operatorname{INST}
      \end{align*}
      
    - Packages that depend on non installable packages become also
      uninstallable.
      \begin{align*}
      \operatorname{ToCConsider} &= (\oob{\operatorname{INST}}\times \universe{})\cap\glb{\operatorname{DEPS}} \\
      \operatorname{UnInstallable} &= \proj{\operatorname{ToConsider}}{1}\\
      \operatorname{UnInstallable} || \operatorname{INST}
      \end{align*}
      
    #+BEGIN_SRC c++
    dependencies(*this,deps_,inst_); 
    #+END_SRC
...
*** Parser, translator and reducer statistics
    This secion presents the times (in seconds) that tools like the
    parser, the translator and the reducer spent in the set of
    instances. The last column present the group to which the
    particular instance belongs. The /State/ column is the output of
    the reducer on the particular instance. This can be:
    ...
    As he results can change with the evolution of the tools, a
    subsection is made with the date in which the benchmark was
    applied.
...
**** <2011-09-09 Fri>
     ...
     The following times were taken *on reduced instances*. This
     means, he instances produced by the reducer.
       
     An interesting aspect of this test is that it uses the same bdd
     manager for all the instances. This is, no new manager is creaed
     between two different instances. Even more interesting is the
     fact that the memmory used during all the run of the program
     never goes more than 80 megabytes.
...
*** Solver times
    Memory meassurements for loading the relations of the different
    instances in gecode. The first column is the instance name, the
    second one is the time in seconds for reading the input using the
    /KCUDF/. The nex two columns are the Gecode memmory consumption
    and the solving time. The las group of columns contains the
    memmory in bytes used by the /BDD/ manager, the time (in
    milliseconds) spent in garbage colloections and the actual number
    of garbage collections performed.

**** <2011-09-12 Mon>
     #+ATTR_LaTeX: longtable
     |------------------------------------+----------+----------+----------+----------+---------+----|
     | Instance                           |     Read | Mem (KB) |    Solve | Mem (MB) | GC time | GC |
     |------------------------------------+----------+----------+----------+----------+---------+----|
     | /                                  |        < |        < |        > |        < |         |  > |
     | ark                                |  16.5548 |   321.86 | 0.001795 |    57.27 |    2.39 | 36 |
     | audacious-plugins-extra            |  16.7979 |   321.86 | 0.001921 |    56.43 |   2.413 | 36 |
     | codebreaker                        |  14.6579 |   305.86 | 0.001779 |    56.06 |   2.043 | 32 |
     | dpkg-dev                           |  16.7917 |   321.86 | 0.001769 |    56.87 |   2.395 | 36 |
     | dwww                               |  16.7595 |   321.86 | 0.001893 |    56.18 |   2.417 | 36 |
     | epiphany-browser                   |  16.6794 |   321.86 | 0.001806 |    56.87 |   2.381 | 36 |
     | evolution-dev                      |  16.7397 |   321.86 | 0.001813 |    57.15 |   2.381 | 36 |
     | fcron                              |  15.0159 |   305.86 | 0.001867 |    55.65 |   2.046 | 32 |
     | gnash-common-opengl                |  16.8481 |   321.86 |  0.00183 |    55.99 |   2.417 | 36 |
     | gnochm                             |  16.7433 |   321.86 | 0.001863 |    56.71 |   2.412 | 36 |
     | gnome-panel-data                   |   16.756 |   321.86 | 0.001859 |    57.06 |   2.379 | 36 |
     | gpc-4.1                            |  14.0108 |   289.86 | 0.001692 |    55.65 |   1.986 | 31 |
     | gtwitter                           |  14.3458 |   305.86 | 0.002219 |    54.99 |   1.925 | 31 |
     | guake                              |  14.0421 |   297.86 | 0.001973 |    54.78 |   1.886 | 30 |
     | hal-cups-utils                     |  15.2316 |   305.86 | 0.001793 |    56.43 |   2.151 | 33 |
     | idle                               |  16.8118 |   321.86 | 0.001808 |    55.81 |   2.414 | 36 |
     | inetutils-syslogd                  |   16.749 |   321.86 | 0.001785 |    57.12 |   2.384 | 36 |
     | kbd                                |  17.2297 |   321.86 | 0.001808 |    55.81 |   2.442 | 36 |
     | kblocks                            |  16.9877 |   321.86 | 0.001877 |    56.02 |   2.444 | 36 |
     | kdebase-bin                        |  17.2108 |   321.86 | 0.001755 |    56.09 |    2.44 | 36 |
     | kdemultimedia-kio-plugins          |  17.0889 |   321.86 |  0.00181 |    56.34 |    2.42 | 36 |
     | kdessh                             |  17.1147 |   321.86 | 0.001779 |    56.87 |   2.403 | 36 |
     | kdesudo                            |  17.0233 |   321.86 | 0.001816 |    56.59 |   2.339 | 35 |
     | kfilereplace                       |  17.2154 |   321.86 | 0.001758 |    56.40 |   2.424 | 36 |
     | kjumpingcube                       |  16.6671 |   321.86 | 0.001751 |    57.27 |   2.409 | 36 |
     | kmines                             |   17.006 |   321.86 | 0.001879 |    56.24 |   2.433 | 36 |
     | knetwalk                           |  16.7861 |   321.86 | 0.001819 |    56.96 |   2.412 | 36 |
     | kommander                          |  16.6853 |   321.86 | 0.001867 |    57.15 |   2.379 | 36 |
     | konq-plugins                       |   16.949 |   321.86 | 0.001779 |    56.34 |   2.414 | 36 |
     | kphotoalbum                        |  17.0421 |   321.86 |  0.00177 |    56.02 |   2.426 | 36 |
     | ksystemlog                         |  16.9717 |   321.86 | 0.001853 |    57.18 |   2.397 | 36 |
     | ktouch                             |  16.8874 |   321.86 |  0.00176 |    55.93 |   2.429 | 36 |
     | kwin-style-crystal                 |  16.7946 |   321.86 | 0.001825 |    56.93 |   2.382 | 36 |
     | libbsd-resource-perl               |   16.903 |   321.86 | 0.001746 |    56.87 |   2.373 | 36 |
     | libcatalyst-perl                   |  16.8183 |   321.86 |  0.00184 |    57.12 |   2.382 | 36 |
     | libcurses-perl                     |  16.8517 |   321.86 | 0.001845 |    56.27 |   2.424 | 36 |
     | libcyrus-imap-perl22               |  15.2814 |   305.86 |  0.00177 |    56.37 |   2.157 | 33 |
     | libdbd-pg-perl                     |  16.9002 |   321.86 | 0.001843 |    56.77 |   2.395 | 36 |
     | libevolution3.0-cil                |  15.1186 |   305.86 | 0.001931 |    55.81 |   2.033 | 32 |
     | libgnokii4                         |   16.849 |   321.86 | 0.001741 |    57.21 |   2.389 | 36 |
     | libhtml-parser-perl                |  16.7357 |   321.86 | 0.001856 |    56.09 |   2.322 | 35 |
     | libmail-spf-perl                   |  16.7392 |   321.86 | 0.001857 |    57.02 |   2.321 | 35 |
     | libmono-addins-gui0.2-cil          |  16.9186 |   321.86 | 0.001788 |    56.34 |    2.42 | 36 |
     | libmono-relaxng1.0-cil             |    16.76 |   321.86 | 0.001842 |    56.81 |   2.379 | 36 |
     | libnkf-perl                        |  15.1421 |   305.86 |  0.00174 |    55.56 |   2.087 | 33 |
     | libnspr4-dev                       |   16.745 |   321.86 | 0.001779 |    57.24 |   2.387 | 36 |
     | libnss3-1d                         |  16.5166 |   321.86 | 0.001872 |    57.27 |   2.382 | 36 |
     | libpango-perl                      |  14.5964 |   305.86 | 0.001795 |    56.06 |   2.048 | 32 |
     | libpoe-perl                        |  17.1124 |   321.86 | 0.001824 |    56.93 |   2.385 | 36 |
     | libtemplate-provider-encoding-perl |  16.5274 |   321.86 | 0.001919 |    56.59 |   2.319 | 35 |
     | libwnck2.20-cil                    |  16.8014 |   321.86 | 0.001836 |    56.59 |   2.326 | 35 |
     | lirc                               |  15.2175 |   305.86 | 0.001842 |    56.43 |   2.186 | 34 |
     | mercurial-common                   |  16.8303 |   321.86 | 0.001808 |    56.12 |   2.412 | 36 |
     | mono-debugger                      |  16.9205 |   321.86 | 0.001891 |    56.31 |    2.42 | 36 |
     | mono-gac                           |  16.7622 |   321.86 | 0.001796 |    56.99 |   2.388 | 36 |
     | mono-mcs                           |  16.9094 |   321.86 | 0.001858 |    56.77 |   2.397 | 36 |
     | monodevelop-nunit                  |  16.8351 |   321.86 | 0.001784 |    56.31 |   2.414 | 36 |
     | openoffice.org-draw                |  17.0151 |   321.86 | 0.001858 |    56.09 |    2.33 | 35 |
     | openoffice.org-l10n-da             |  16.7194 |   321.86 | 0.001862 |    56.93 |   2.408 | 36 |
     | openoffice.org-l10n-nl             |  17.0154 |   321.86 | 0.001775 |    55.81 |   2.426 | 36 |
     | pidgin-plugin-pack                 |  14.8887 |   305.86 |   0.0018 |    55.77 |   2.029 | 32 |
     | plasma-dataengines-workspace       |  17.0269 |   321.86 | 0.001836 |    56.56 |   2.423 | 36 |
     | plasma-widgets-workspace           |  17.2447 |   321.86 | 0.001821 |    55.87 |   2.431 | 36 |
     | pybackpack                         |  17.0736 |   321.86 | 0.001919 |    56.34 |   2.429 | 36 |
     | python-cairo                       |  17.2538 |   321.86 | 0.001824 |    56.84 |   2.437 | 36 |
     | python-hippocanvas                 |  14.7484 |   305.86 | 0.001773 |    55.84 |   2.029 | 32 |
     | r-cran-car                         |  16.9271 |   321.86 | 0.001774 |    57.30 |   2.403 | 36 |
     | r-cran-lme4                        |  17.0993 |   321.86 | 0.001876 |    56.12 |   2.421 | 36 |
     | r-cran-rodbc                       |  16.8607 |   321.86 | 0.001833 |    56.96 |   2.383 | 36 |
     | rand0c02bc                         |  7.49036 |   177.86 | 0.002227 |    49.96 |   0.859 | 16 |
     | rand2d8467                         |  7.58557 |   185.86 | 0.002393 |    49.52 |   0.871 | 16 |
     | rand43bd88                         |  7.25721 |   185.86 | 0.002675 |    50.49 |   0.875 | 16 |
     | rand4bf850                         |  6.56145 |   161.86 | 0.002719 |    48.74 |   0.753 | 14 |
     | rand65d021                         |  7.70242 |   177.86 | 0.002478 |    50.46 |   0.873 | 16 |
     | rand8177e7                         |  7.74492 |   185.86 | 0.003462 |    51.77 |   0.944 | 17 |
     | rand9e35f0                         |  0.86094 |    41.86 | 0.000466 |    22.05 |    0.09 |  3 |
     | randa09fe9                         |  7.32208 |   169.86 | 0.002544 |    50.52 |   0.889 | 16 |
     | randa13f2d                         | 0.775037 |    41.86 | 0.000427 |    21.64 |   0.059 |  2 |
     | randa6997e                         |  8.83075 |   193.86 | 0.002484 |    50.11 |   1.081 | 19 |
     | randaefc7c                         |  7.67682 |   161.86 | 0.001305 |    50.09 |    0.91 | 16 |
     | randb87834                         |  7.52046 |   177.86 | 0.002825 |    50.15 |    0.87 | 16 |
     | randbccf2d                         |  6.98394 |   169.86 | 0.002219 |    50.37 |   0.793 | 15 |
     | randd1cab2                         |  7.58847 |   177.86 | 0.002758 |    50.02 |   0.861 | 16 |
     | randf7d077                         |  7.84011 |   185.86 | 0.002566 |    49.93 |   0.944 | 17 |
     | sonata                             |  17.0768 |   321.86 | 0.001798 |    56.43 |   2.421 | 36 |
     | synaptic                           |  16.9266 |   321.86 | 0.001785 |    56.99 |   2.394 | 36 |
     | system-config-printer              |  16.7792 |   321.86 | 0.001843 |    56.77 |   2.386 | 36 |
     | tasque                             |  16.8754 |   321.86 | 0.001831 |    56.02 |   2.427 | 36 |
     | texlive-generic-recommended        |  16.7509 |   321.86 |  0.00177 |    56.12 |   2.313 | 35 |
     | txt2html                           |  16.5904 |   321.86 | 0.001898 |    56.99 |   2.331 | 35 |
     | unattended-upgrades                |  16.7827 |   321.86 | 0.001868 |    55.87 |   2.411 | 36 |
     | upstart-compat-sysv                |  15.4358 |   305.86 | 0.001731 |    56.37 |   2.198 | 34 |
     | xvfb                               |  16.7687 |   321.86 | 0.001901 |    57.30 |   2.377 | 36 |
     |------------------------------------+----------+----------+----------+----------+---------+----|


**** TODO <2011-09-12 Mon> 					   :noexport:
     This benchmarks resulted in strange results. They should be
     compared against the ones the reducer output no solution for.

     #+ATTR_LaTeX: longtable
     |------------+------+----------+----------+----------+--------+----|
     | Instance   | Read |      Mem |    Solve |  Mem (b) | GC (s) | GC |
     |------------+------+----------+----------+----------+--------+----|
     | randa491d5 | 6000 | 0.007771 | 0.000117 |  9493856 |      0 |  0 |
     | randa65dfb | 6000 | 0.007259 | 0.000113 |  9493856 |      0 |  0 |
     | randa65f48 | 6000 |   0.0071 | 0.000119 |  9493856 |      0 |  0 |
     | randab5933 | 6000 | 0.007428 | 0.000118 |  9493856 |      0 |  0 |
     | randaef248 | 6000 | 0.006946 | 0.000107 |  9493856 |      0 |  0 |
     | randb3c96d | 6000 | 0.007281 | 0.000114 |  9493856 |      0 |  0 |
     | randbbb4ba | 6000 | 0.007497 | 0.000116 |  9493856 |      0 |  0 |
     | randcbf550 | 6000 | 0.007458 | 0.000112 |  9493856 |      0 |  0 |
     | randcef367 | 6000 | 0.006931 | 0.000109 |  9493856 |      0 |  0 |
     | randd6bc17 | 6000 | 0.090994 | 0.000122 | 13007072 |      0 |  0 |
     | randdc506f | 6000 | 0.007471 | 0.000112 |  9493856 |      0 |  0 |
     | randdd5509 | 6000 | 0.007169 | 0.000108 |  9493856 |      0 |  0 |
     | rand03b0b5 | 6000 | 0.007735 | 0.000109 |  9493856 |      0 |  0 |
     | rand13056c | 6000 | 0.007578 | 0.000113 |  9493856 |      0 |  0 |
     | rand144372 | 6000 | 0.007021 |  0.00011 |  9493856 |      0 |  0 |
     | rand1958fb | 6000 | 0.007017 | 0.000106 |  9493856 |      0 |  0 |
     | rand199fd6 | 6000 | 0.007246 | 0.000111 |  9493856 |      0 |  0 |
     | rand2d77b5 | 6000 | 0.007559 | 0.000113 |  9493856 |      0 |  0 |
     | rand2fa1f8 | 6000 | 0.007479 | 0.000118 |  9493856 |      0 |  0 |
     | rand35afbf | 6000 |   0.0072 |  0.00011 |  9493856 |      0 |  0 |
     | rand35d0c8 | 6000 | 0.007061 | 0.000109 |  9493856 |      0 |  0 |
     | rand396cf8 | 6000 | 0.007428 | 0.000116 |  9493856 |      0 |  0 |
     | rand49e642 | 6000 | 0.007667 | 0.000121 |  9493856 |      0 |  0 |
     | rand56b853 | 6000 |  0.00744 | 0.000122 |  9493856 |      0 |  0 |
     | rand59f7dd | 6000 |  0.00733 | 0.000112 |  9493856 |      0 |  0 |
     | rand5c0b12 | 6000 | 0.007245 | 0.000113 |  9493856 |      0 |  0 |
     | rand601ba8 | 6000 | 0.007056 | 0.000111 |  9493856 |      0 |  0 |
     | rand659e05 | 6000 | 0.007196 | 0.000111 |  9493856 |      0 |  0 |
     | rand682c92 | 6000 | 0.007301 | 0.000122 |  9493856 |      0 |  0 |
     | rand6c4744 | 6000 | 0.007024 | 0.000112 |  9493856 |      0 |  0 |
     | rand6d579e | 6000 | 0.007029 | 0.000118 |  9493856 |      0 |  0 |
     | rand6f4585 | 6000 |  0.00738 | 0.000119 |  9493856 |      0 |  0 |
     | rand7e9372 | 6000 | 0.007097 | 0.000183 |  9493856 |      0 |  0 |
     | rand88811a | 6000 | 0.007486 | 0.000122 |  9493856 |      0 |  0 |
     | rand902db1 | 6000 | 0.007177 | 0.000109 |  9493856 |      0 |  0 |
     | rand936b0d | 6000 | 0.006878 | 0.000113 |  9493856 |      0 |  0 |
     | rande2165e | 6000 | 0.007125 | 0.000106 |  9493856 |      0 |  0 |
     | rande22180 | 6000 | 0.006967 |  0.00011 |  9493856 |      0 |  0 |
     | rande89f80 | 6000 | 0.007378 |  0.00011 |  9493856 |      0 |  0 |
     | randea4da3 | 6000 | 0.007117 | 0.000113 |  9493856 |      0 |  0 |
     | randee102b | 6000 | 0.007471 | 0.000121 |  9493856 |      0 |  0 |
     | randfe95e3 | 6000 | 0.007809 | 0.000119 |  9493856 |      0 |  0 |
     |------------+------+----------+----------+----------+--------+----|
...
* Unique quantification
  ...
  |       1 |       0 |
  |---------+---------|
  From it we are interested in all the elements in the first columns
  that are /unique/. By simple inspection we know that those elements
  are 0 and 1. This apparently simple operation is the core of many
  constraints and therefore it needs to be performed efficiently. This
  section defines the semntics of this operation and its
  implementation at the /BDD/ level.
    
  The operation takes as arguments a relation $R \subseteq
  \universe{n}$ and a set of columns indices $I$. The operation then
  abstracts the column corresponding to the indices and returnsa
  relation for which the non abstracted part is /unique/.

  It is important to signal that the operation returns a relation of
  the same arity as the input one. For the previous case, the result is:
  |---------+---------|
  | Col_{1} | Col_{0} |
  |---------+---------|
  | 0       | -       |
  ...
  The presence of "-" in the last column of the table indicates that
  all the elements of the universe set can be possible values at that
  place. This is actually what represents the notion of quantification
  in the operation.

  The same reasoning is used when the relation has a greater
  arity. For instance, by taking the relation:
   |---------+---------+---------|
   | Col_{2} | Col_{1} | Col_{0} |
   |---------+---------+---------|
   |       0 |       0 |       1 |
   |       0 |       0 |       2 |
   |       0 |       1 |       1 |
   |       1 |       2 |       3 |
   |---------+---------+---------|
  and quantifying on the last column we get the relation:
  |---------+---------+---------|
  | Col_{2} | Col_{1} | Col_{0} |
  |---------+---------+---------|
  |       0 |       1 | -       |
  |       1 |       2 | -       |
  |---------+---------+---------|
  and the same relation but quantifying on the two last columns will result in:
  |---------+---------+---------|
  | Col_{2} | Col_{1} | Col_{0} |
  |---------+---------+---------|
  |       1 | -       | -       |
...
** TODO To complete
*** TODO Formal definition semantics
*** TODO BDD representation
*** Under the hood
   
   Suppose the following binary relation
   \begin{align*}
   R = \{\pair{0}{3},\pair{1}{3},\pair{2}{3},\pair{3}{3}\}
   \end{align*}

   In order to represent all the elements in the tuples we need at most
   2 bits. Therfore, every column will be represented by two /BDD/
   variables. The following /BDD/ contains the representation of that
   relation:
   #+begin_dot binrel-quant.pdf -Tpdf
   digraph G {
   ...
   The actual encoding of the pairs is:
   |-------------+-------+-------+-------+-------|
   | Pair        | V_{0} | V_{1} | V_{2} | V_{3} |
   |-------------+-------+-------+-------+-------|
   | \pair{0}{3} |     0 |     1 |     0 |     1 |
   | \pair{2}{3} |     1 |     1 |     0 |     1 |
   | \pair{1}{3} |     0 |     1 |     1 |     1 |
   | \pair{3}{3} |     1 |     1 |     1 |     1 |
   |-------------+-------+-------+-------+-------|
   Where the variables $V_{1}$ and $V_{3}$ are used to encode the
   second term of every pair while variables $V_{0}$ and $V_{2}$ are
   used for the first one. The fact that variables $V_{0}$ and $V_{2}$
   are not present in the resulting /BDD/ is reflected in the encoding
   by considering either value is possible. 

   To perform the quantification we need to provide a /cube/. This is a
   special /BDD/ that contains the variables that we want to quantify
   on. For quantifying on the first column we use the cube:

   #+begin_dot binrel-quant-cube.pdf -Tpdf
   digraph G {
   0 [shape=box, label="0", style=filled, shape=box, height=0.3, width=0.3];
   1 [shape=box, label="1", style=filled, shape=box, height=0.3, width=0.3];
   10 [label="0"];
   ...
   The /BDD/ resulting from the unique quantification of relation $R$
   with respect to the variables in the cube above is /False/. This is
   in fact the representation of the empty relation. 

   Now consider the following relation:
   \begin{align*}
   S = \{\pair{0}{3},\pair{1}{3},\pair{2}{3},\pair{3}{3}\}
   \end{align*}
   
   Representing by the /BDD/:
   #+begin_dot binrel-quant2.pdf -Tpdf
   digraph G {
   0 [shape=box, label="0", style=filled, shape=box, height=0.3, width=0.3];
   1 [shape=box, label="1", style=filled, shape=box, height=0.3, width=0.3];
   23 [label="0"];
   ...
   |--------------------+---+------------------------------------------+----------+---------|
   | unique(r,q)        | = | xor(unique(r0,q1),unique(r1,q1))         | L(r)=0   | L(q)=0  |
   |--------------------+---+------------------------------------------+----------+---------|
   |--------------------+---+------------------------------------------+----------+---------|
   | unique(r0,q1)      | = | ite(unique(FALSE,q1),unique(r01,q1))     | L(r0)=1  | L(q1)=2 |
   | unique(FALSE,q1)   | = | FALSE                                    |          |         |
   | unique(r01,q1)     | = | FALSE                                    | L(r01)=3 | L(q1)=2 |
   |--------------------+---+------------------------------------------+----------+---------|
   | unique(r1,q1)      | = | ite(unique(FALSE,q1),unique(r11,q1))     | L(r1)=1  | L(q1)=2 |
   | unique(FALSE,q1)   | = | FALSE                                    |          |         |
   | unique(r11,q1)     | = | xor(unique(r01,TRUE),unique(FALSE,TRUE)) | L(r11)=2 | L(q1)=2 |
   | unique(r01,TRUE)   | = | r01                                      |          |         |
   ...
   |--------------------+---+----------------------------------+----------+---------|
   |--------------------+---+----------------------------------+----------+---------|
   | unique(r0,q1)      | = | ite(FALSE,FALSE)                 | L(r0)=1  | L(q1)=2 |
   | unique(FALSE,q1)   | = | FALSE                            |          |         |
   | unique(r01,q1)     | = | FALSE                            | L(r01)=3 | L(q1)=2 |
   |--------------------+---+----------------------------------+----------+---------|
   | unique(r1,q1)      | = | ite(FALSE,xor(r01,FALSE))        | L(r1)=1  | L(q1)=2 |
   | unique(FALSE,q1)   | = | FALSE                            |          |         |
   | unique(r11,q1)     | = | xor(r01,FALSE)                   | L(r11)=2 | L(q1)=2 |
   | unique(r01,TRUE)   | = | r01                              |          |         |
   | unique(FALSE,TRUE) | = | FALSE                            |          |         |
   |--------------------+---+----------------------------------+----------+---------|

   |---------------------------+---+---------------------------+----------+---------|
   | unique(r,q)               | = | xor(FALSE,ite(FALSE,r01)) | L(r)=0   | L(q)=0  |
   |---------------------------+---+---------------------------+----------+---------|
   |---------------------------+---+---------------------------+----------+---------|
   | unique(r0,q1)             | = | FALSE                     | L(r0)=1  | L(q1)=2 |
   | unique(FALSE,q1)          | = | FALSE                     |          |         |
   | unique(r01,q1)            | = | FALSE                     | L(r01)=3 | L(q1)=2 |
   |---------------------------+---+---------------------------+----------+---------|
   | unique(r1,q1)             | = | ite(FALSE,r01)            | L(r1)=1  | L(q1)=2 |
   | unique(FALSE,q1)          | = | FALSE                     |          |         |
   | unique(r11,q1)            | = | r01                       | L(r11)=2 | L(q1)=2 |
   | unique(r01,TRUE)          | = | r01                       |          |         |
   | unique(FALSE,TRUE)        | = | FALSE                     |          |         |
   | xor(FALSE,ite(FALSE,r01)) | = | ite(FALSE,r01)            |          |         |
   |---------------------------+---+---------------------------+----------+---------|
   
*** Why unique quantification seems to not be what I need?
    The following relation is represented by the /BDD/ bellow it.
    \begin{align*}
    S = \{\pair{0}{3},\pair{1}{3},\pair{2}{3}\}
    \end{align*}
...
*** My own unique quantification
   The following is the /BDD/ representing the relation. 

   #+ATTR_LaTeX: width=0.8\textwidth
   #+begin_dot myunique0.pdf -Tpdf
    digraph G {
0 [shape=box, label="F", style=filled, shape=box, height=0.3, width=0.3];
1 [shape=box, label="T", style=filled, shape=box, height=0.3, width=0.3];
2 [label="0"]
2 -> 3 [style=dotted];
3 [label="1"]
3 -> 0 [style=dotted];
...
* Existential quantification
  This operation takes a relation $R \subseteq \universe{n}$ and a set
  $I$ of column indices and gives as result a relation of the same
  arity in which the columns corresponding to the indices are
  existentially quantified. This is, they can be any value in the
  universe.
